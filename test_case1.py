...
def create_scan_plan(self, driver, tab, name, description, number, position):
    """
    :param name: 方案名称
...
@pytest.mark.parametrize(
    ...
    read_excel_data('test_data.xlsx', 'Sheet2')  # 替换为你的Excel文件路径和Sheet名称
)
...
#     # 判断文件是否存在
#     if os.path.exists('allure_file'):
#         shutil.rmtree('allure_file')
#     print("Cleaned allure_file directory if it existed.")


if __name__ == '__main__':
    # clean_allure_dir()  # 清除 allure_file 目录（如果存在）
    pytest.main(["-s", "-v", "--alluredir=allure_file"])  # 没有自动生成文件
